...
# 1. Update upload_max_filesize to 512M
docker exec -u root $CONTAINER sed -i "s/^upload_max_filesize = .*/upload_max_filesize = 512M/" "$PHP_INI"

# 2. Update post_max_size to 512M
docker exec -u root $CONTAINER sed -i "s/^post_max_size = .*/post_max_size = 512M/" "$PHP_INI"
